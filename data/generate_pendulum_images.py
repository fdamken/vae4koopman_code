...
def sample_dynamics() -> Tuple[np.ndarray, np.ndarray]:
    ...
    for _ in range(0, N):
        ...
        sample_sequences.append(sci.solve_ivp(ode, (0, t_final), initial_value, t_eval=np.arange(0, t_final, h), method='Radau').y.T)
    sample_sequences = np.asarray(sample_sequences)
    sample_sequences_noisy = sample_sequences + np.random.multivariate_normal(np.array([0.0]), np.array([[observation_cov]]), size=sample_sequences.shape).reshape(sample_sequences.shape)
    return sample_sequences, sample_sequences_noisy


def save_pendulum_image(tau_p: float, phi_p: float, prefix: str) -> str:
    ...
    path = '%s/%s-%06.3f.bmp' % (out_dir, prefix, tau_p)
